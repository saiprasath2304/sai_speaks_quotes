Privacy Policy for Sai Speaks

Effective Date: [Date you publish]

Sai Speaks ("the App") respects the privacy of its users. 

Information Collection:
- The App does not collect, store, or share any personal user data.
- The App checks internet connectivity solely to enable video content playback.

Information Usage:
- No personal information is collected or transmitted to our servers.

Third-Party Services:
- The App may use internet connectivity to load videos from publicly available sources.

Children’s Privacy:
- The App is suitable for users of all ages and does not knowingly collect information from children.

Changes to this Policy:
- This Privacy Policy may be updated occasionally. Users are encouraged to review it periodically.

Contact Us:
- If you have any questions about this Privacy Policy, please get in touch with us at sp.pers23@gmail.com.

